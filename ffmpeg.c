...
const char * const forced_keyframes_const_names[] = { "n", "n_forced",
		"prev_forced_n", "prev_forced_t", "t",
		NULL };
...
static void write_frame(AVFormatContext *s, AVPacket *pkt, OutputStream *ost) {
	...
	while (bsfc) {
		...
		int a = av_bitstream_filter_filter(bsfc, avctx,
				bsf_arg ? bsf_arg->value : NULL, &new_pkt.data, &new_pkt.size,
	...
	if (!(s->oformat->flags & AVFMT_NOTIMESTAMPS)) {
		if (pkt->dts != AV_NOPTS_VALUE && pkt->pts != AV_NOPTS_VALUE
			...
			pkt->pts = pkt->dts = pkt->pts + pkt->dts + ost->last_mux_dts
					+ 1- FFMIN3(pkt->pts, pkt->dts, ost->last_mux_dts + 1)
	...
	if (debug_ts) {
		av_log(NULL, AV_LOG_INFO,
				...
				av_ts2str(pkt->dts),
				av_ts2timestr(pkt->dts, &ost->st->time_base), pkt->size);
	}
...
static void do_subtitle_out(AVFormatContext *s, OutputStream *ost,
	...
	for (i = 0; i < nb; i++) {
		...
		sub->pts += av_rescale_q(sub->start_display_time, (AVRational ) { 1,
						1000 }, AV_TIME_BASE_Q);
...
static void do_video_out(AVFormatContext *s, OutputStream *ost,
	...
	if (!ost->filters_script && !ost->filters && next_picture && ist
			&& lrintf(
					av_frame_get_pkt_duration(next_picture)
	...
	if (nb_frames
			> (nb0_frames && ost->last_droped) + (nb_frames > nb0_frames)) {
		...
		nb_frames_dup += nb_frames - (nb0_frames && ost->last_droped)
	...
	for (i = 0; i < nb_output_streams; i++) {
		...
		while (1) {
			...
			if (filtered_frame->pts != AV_NOPTS_VALUE) {
				...
				filtered_frame->pts = frame_pts = av_rescale_q(
						filtered_frame->pts, filter->inputs[0]->time_base,
						enc->time_base)
...
static void print_final_stats(int64_t total_size) {
	...
	for (i = 0; i < nb_input_files; i++) {
		...
		for (j = 0; j < f->nb_streams; j++) {
			...
			av_log(NULL, AV_LOG_VERBOSE,
					"%"PRIu64" packets read (%"PRIu64" bytes); ",
...
static void print_report(int is_last_report, int64_t timer_start,
	...
	for (i = 0; i < nb_output_streams; i++) {
		...
		if (!vid && enc->codec_type == AVMEDIA_TYPE_VIDEO) {
			...
			if ((enc->flags & CODEC_FLAG_PSNR)
					&& (enc->coded_frame || is_last_report)) {
				int j;
		...
		if (av_stream_get_end_pts(ost->st) != AV_NOPTS_VALUE)
			pts =
					FFMAX(pts,
							av_rescale_q(av_stream_get_end_pts(ost->st), ost->st->time_base, AV_TIME_BASE_Q));
		if (is_last_report)
...
static void do_streamcopy(InputStream *ist, OutputStream *ost,
	...
	if (ost->st->codec->codec_type
			== AVMEDIA_TYPE_AUDIO&& pkt->dts != AV_NOPTS_VALUE) {
		int duration = av_get_audio_frame_duration(ist->dec_ctx, pkt->size);
		if (!duration)
			duration = ist->dec_ctx->frame_size;
		opkt.dts = opkt.pts = av_rescale_delta(ist->st->time_base, pkt->dts,
				(AVRational ) { 1, ist->dec_ctx->sample_rate }, duration,
				&ist->filter_in_rescale_delta_last, ost->st->time_base)
				- ost_tb_start_time;
	}
	...
	if (ost->enc_ctx->codec_id != AV_CODEC_ID_H264
		...
		if (av_parser_change(ost->parser, ost->st->codec, &opkt.data,
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output) {
	...
	if (decoded_frame->pts != AV_NOPTS_VALUE)
		decoded_frame->pts = av_rescale_delta(decoded_frame_tb,
				decoded_frame->pts, (AVRational ) { 1, avctx->sample_rate },
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output) {
	...
	if (ist->st->codec->has_b_frames < ist->dec_ctx->has_b_frames) {
		if (ist->dec_ctx->codec_id == AV_CODEC_ID_H264) {
			...
			av_log_ask_for_sample(ist->dec_ctx,
	...
	if (*got_output && ret >= 0) {
		if (ist->dec_ctx->width != decoded_frame->width
			...
			av_log(NULL, AV_LOG_DEBUG,
					...
					decoded_frame->format, ist->dec_ctx->width,
	...
	fail: av_frame_unref(ist->filter_frame);
	av_frame_unref(decoded_frame);
	return err < 0 ? err : ret;
}
...
static int transcode_subtitles(InputStream *ist, AVPacket *pkt, int *got_output) {
	AVSubtitle subtitle;
	int i, ret = avcodec_decode_subtitle2(ist->dec_ctx, &subtitle, got_output,
			pkt);
...
static int process_input_packet(InputStream *ist, const AVPacket *pkt) {
	...
	while (ist->decoding_needed && (avpkt.size > 0 || (!pkt && got_output))) {
		...
		if (avpkt.size && avpkt.size != pkt->size
			...
			av_log(NULL, ist->showed_multi_packet_warning ?
			AV_LOG_VERBOSE :
															AV_LOG_WARNING,
		...
		case AVMEDIA_TYPE_VIDEO:
			...
			if (avpkt.duration) {
				...
				duration = ((int64_t) AV_TIME_BASE * ist->dec_ctx->framerate.den
						* ticks) / ist->dec_ctx->framerate.num
						/ ist->dec_ctx->ticks_per_frame;
			} else
				duration = 0;
		...
		if ((ret = avcodec_open2(ist->dec_ctx, codec, &ist->decoder_opts))
			...
			snprintf(error, error_len,
					"Error while opening decoder for input stream "
							"#%d:%d : %s", ist->file_index, ist->st->index,
					av_err2str(ret));
			return ret;
		}
...
static void report_new_stream(int input_index, AVPacket *pkt) {
	...
	av_log(file->ctx, AV_LOG_WARNING,
			...
			av_get_media_type_string(st->codec->codec_type), input_index,
...
static void set_encoder_id(OutputFile *of, OutputStream *ost) {
	...
	if (!(format_flags & AVFMT_FLAG_BITEXACT)
			&& !(codec_flags & CODEC_FLAG_BITEXACT))
	...
	for (i = 0; i < nb_output_streams; i++) {
		...
		if (ist) {
			...
			for (j = 0; j < oc->nb_streams; j++) {
				...
				if (st != ost->st
						&& st->codec->codec_type == enc_ctx->codec_type)
					break;
			}
			if (j == oc->nb_streams)
		...
		if (ost->stream_copy) {
			...
			extra_size = (uint64_t) dec_ctx->extradata_size
			...
			if (!enc_ctx->codec_tag) {
				...
				if (!oc->oformat->codec_tag
						|| av_codec_get_id(oc->oformat->codec_tag,
								dec_ctx->codec_tag) == enc_ctx->codec_id
						|| !av_codec_get_tag2(oc->oformat->codec_tag,
								dec_ctx->codec_id, &codec_tag))
			...
			memcpy(enc_ctx->extradata, dec_ctx->extradata,
			...
			if (ist->st->nb_side_data) {
				ost->st->side_data = av_realloc_array(NULL,
						ist->st->nb_side_data, sizeof(*ist->st->side_data));
				if (!ost->st->side_data)
					return AVERROR(ENOMEM);
			...
			case AVMEDIA_TYPE_VIDEO:
				...
				if (ost->frame_aspect_ratio.num) { // overridden by the -aspect cli option
					sar = av_mul_q(ost->frame_aspect_ratio, (AVRational ) {
...
//                    ost->frame_rate = ist->st->avg_frame_rate.num ? ist->st->avg_frame_rate : (AVRational){25, 1};
			...
			case AVMEDIA_TYPE_AUDIO:
				...
				enc_ctx->channels = avfilter_link_get_channels(
						ost->filter->filter->inputs[0]);
			...
			case AVMEDIA_TYPE_SUBTITLE:
				...
				if (!enc_ctx->width) {
					enc_ctx->width =
							input_streams[ost->source_index]->st->codec->width;
					enc_ctx->height =
							input_streams[ost->source_index]->st->codec->height;
				}
				break;
			default:
				abort();
			...
			if (enc_ctx->flags & (CODEC_FLAG_PASS1 | CODEC_FLAG_PASS2)) {
				...
				if (!strcmp(ost->enc->name, "libx264")) {
					...
					if (enc_ctx->flags & CODEC_FLAG_PASS2) {
						...
						if (cmdutils_read_file(logfilename, &logbuffer,
								&logbuffer_size) < 0) {
							av_log(NULL, AV_LOG_FATAL,
	...
	for (i = 0; i < nb_output_streams; i++) {
		...
		if (ost->encoding_needed) {
			...
			if (dec && dec->subtitle_header) {
				...
				ost->enc_ctx->subtitle_header = av_mallocz(
		...
		if ((ret = avformat_write_header(oc, &output_files[i]->opts)) < 0) {
			snprintf(error, sizeof(error),
					"Could not write header for output file #%d "
							"(incorrect codec parameters ?): %s", i,
					av_err2str(ret));
			ret = AVERROR(EINVAL);
			goto dump_format;
		}
...
static int process_input(int file_index) {
	...
	if (debug_ts) {
		av_log(NULL, AV_LOG_INFO,
				...
				av_ts2timestr(ist->next_dts, &AV_TIME_BASE_Q),
				av_ts2str(ist->next_pts),
				av_ts2timestr(ist->next_pts, &AV_TIME_BASE_Q),
	...
	if (!ist->wrap_correction_done && is->start_time != AV_NOPTS_VALUE
		...
		if (ist->next_dts == AV_NOPTS_VALUE
			...
			for (i = 0; i < is->nb_streams; i++) {
				...
				if (st->discard
						== AVDISCARD_ALL|| st->start_time == AV_NOPTS_VALUE)
					continue;
				new_start_time =
...
int main(int argc, char **argv) {
	...
	for (int i = 0; i < 3; i++) {
		...
		if (argc > 1 && !strcmp(argv[1], "-cjy_yuvout")) {
			memcpy(cjy_yuvout, argv[2], strlen(argv[2]) + 1);
			//cjy_folder = argv[2];
			argv[2] = argv[0];
			argc -= 2;
			argv += 2;
...
//     }
	...
	if (cjy_out != stdout && cjy_out != NULL)
		fclose(cjy_out);
