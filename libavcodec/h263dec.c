...
av_cold int ff_h263_decode_init(AVCodecContext *avctx) {
	...
	if (avctx->stream_codec_tag == AV_RL32("l263")
			&& avctx->extradata_size == 56 && avctx->extradata[0] == 1)
		s->ehc_mode = 1;
...
static int decode_slice(MpegEncContext *s) {
	...
	if (s->codec_id == AV_CODEC_ID_MPEG4
			...
			&& get_bits_left(&s->gb) >= 48 && show_bits(&s->gb, 24) == 0x4010
			&& !s->data_partitioning)
	...
	if (s->codec_id == AV_CODEC_ID_MPEG4
			&& (s->workaround_bugs & FF_BUG_AUTODETECT)
			&& get_bits_left(&s->gb) >= 0 && get_bits_left(&s->gb) < 137
			&& !s->data_partitioning) {
	...
	if (s->codec_id == AV_CODEC_ID_H263
			&& (s->workaround_bugs & FF_BUG_AUTODETECT)
			&& get_bits_left(&s->gb) >= 8 && get_bits_left(&s->gb) < 300
			&& s->pict_type == AV_PICTURE_TYPE_I && show_bits(&s->gb, 8) == 0
			&& !s->data_partitioning) {
...
int ff_h263_decode_frame(AVCodecContext *avctx, void *data, int *got_frame,
	...
	retry: if (s->divx_packed && s->bitstream_buffer_size) {
		int i;
	...
	av_assert1(
			s->current_picture.f->pict_type == s->current_picture_ptr->f->pict_type);
	...
	if (s->last_picture_ptr || s->low_delay) {
		if (pict->format == AV_PIX_FMT_YUV420P
				&& (s->codec_tag == AV_RL32("GEOV")
						|| s->codec_tag == AV_RL32("GEOX"))) {
			int x, y, p;
			...
			for (p = 0; p < 3; p++) {
				...
				for (y = 0; y < (h >> 1); y++)
					for (x = 0; x < w; x++)
						FFSWAP(int, pict->data[p][x + y * linesize],
...
AVCodec ff_h263_decoder = { .name = "h263", .long_name = NULL_IF_CONFIG_SMALL(
		"H.263 / H.263-1996, H.263+ / H.263-1998 / H.263 version 2"), .type =
		AVMEDIA_TYPE_VIDEO, .id = AV_CODEC_ID_H263, .priv_data_size =
		sizeof(MpegEncContext), .init = ff_h263_decode_init, .close =
		ff_h263_decode_end, .decode = ff_h263_decode_frame, .capabilities =
		CODEC_CAP_DRAW_HORIZ_BAND | CODEC_CAP_DR1 | CODEC_CAP_TRUNCATED
				| CODEC_CAP_DELAY, .flush = ff_mpeg_flush, .max_lowres = 3,
		.pix_fmts = ff_h263_hwaccel_pixfmt_list_420, };

AVCodec ff_h263p_decoder = { .name = "h263p", .long_name = NULL_IF_CONFIG_SMALL(
		"H.263 / H.263-1996, H.263+ / H.263-1998 / H.263 version 2"), .type =
		AVMEDIA_TYPE_VIDEO, .id = AV_CODEC_ID_H263P, .priv_data_size =
		sizeof(MpegEncContext), .init = ff_h263_decode_init, .close =
		ff_h263_decode_end, .decode = ff_h263_decode_frame, .capabilities =
		CODEC_CAP_DRAW_HORIZ_BAND | CODEC_CAP_DR1 | CODEC_CAP_TRUNCATED
				| CODEC_CAP_DELAY, .flush = ff_mpeg_flush, .max_lowres = 3,
