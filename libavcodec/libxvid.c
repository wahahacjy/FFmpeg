...
static int xvid_ff_2pass_create(xvid_plg_create_t *param, void **handle) {
	...
	snprintf(BUFFER_CAT(log), BUFFER_REMAINING(log),
			...
			XVID_VERSION_MAJOR(XVID_VERSION), XVID_VERSION_MINOR(XVID_VERSION),
...
static int xvid_ff_2pass_before(struct xvid_context *ref,
	...
	/* Init values */
	motion_remove = ~XVID_ME_CHROMA_PVOP & ~XVID_ME_CHROMA_BVOP
			& ~XVID_ME_EXTSEARCH16 & ~XVID_ME_ADVANCEDDIAMOND16;
	motion_replacements = XVID_ME_FAST_MODEINTERPOLATE |
	XVID_ME_SKIP_DELTASEARCH |
	XVID_ME_FASTREFINE16 |
	XVID_ME_BFRAME_EARLYSTOP;
	vop_remove = ~XVID_VOP_MODEDECISION_RD & ~XVID_VOP_FAST_MODEDECISION_RD
...
static int xvid_ff_2pass_after(struct xvid_context *ref, xvid_plg_data_t *param) {
	...
	snprintf(BUFFER_CAT(log), BUFFER_REMAINING(log), "%c %d %d %d %d %d %d\n",
			frame_type, param->stats.quant, param->stats.kblks,
			param->stats.mblks, param->stats.ublks, param->stats.length,
			param->stats.hlength);
...
static int xvid_strip_vol_header(AVCodecContext *avctx, AVPacket *pkt,
	...
	for (i = 0; i < header_len - 3; i++) {
		if (pkt->data[i] == 0x00 && pkt->data[i + 1] == 0x00
				&& pkt->data[i + 2] == 0x01 && pkt->data[i + 3] == 0xB6) {
			vo_len = i;
			break;
		}
	}
...
static av_cold int xvid_encode_init(AVCodecContext *avctx) {
	...
	if (avctx->height <= 16) {
		if (avctx->thread_count < 2) {
			...
			av_log(avctx, AV_LOG_ERROR, "Too small height for threads > 1.");
			ret = AVERROR(EINVAL);
			goto fail;
		}
	}
	...
	av_assert0(
			xvid_enc_create.num_plugins + (!!x->ssim) + (!!x->variance_aq) + (!!x->lumi_aq) <= FF_ARRAY_ELEMS(plugins));
...
static int xvid_encode_frame(AVCodecContext *avctx, AVPacket *pkt,
	...
	if ((ret = ff_alloc_packet2(avctx, pkt,
			mb_width * mb_height * MAX_MB_BYTES + FF_MIN_BUFFER_SIZE)) < 0)
		return ret;
	...
	if (avctx->sample_aspect_ratio.num < 0
		...
		av_reduce(&avctx->sample_aspect_ratio.num,
				&avctx->sample_aspect_ratio.den, avctx->sample_aspect_ratio.num,
				avctx->sample_aspect_ratio.den, 255);
	}
	...
	/* Encode */
	xerr = xvid_encore(x->encoder_handle, XVID_ENC_ENCODE, &xvid_enc_frame,
			&xvid_enc_stats);
		...
		*got_packet = 1;
		...
		if (xvid_enc_frame.out_flags & XVID_KEYFRAME) {
			...
			if (x->quicktime_format)
				return xvid_strip_vol_header(avctx, pkt, xvid_enc_stats.hlength,
						xerr);
		} else
			p->key_frame = 0;
...
static const AVOption options[] = { { "lumi_aq", "Luminance masking AQ", OFFSET(
		lumi_aq), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, 1, VE }, { "variance_aq",
		"Variance AQ", OFFSET(variance_aq), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, 1,
		VE }, { "ssim", "Show SSIM information to stdout", OFFSET(ssim),
		AV_OPT_TYPE_INT, { .i64 = 0 }, 0, 2, VE, "ssim" }, { "off", NULL, 0,
		AV_OPT_TYPE_CONST, { .i64 = 0 }, INT_MIN, INT_MAX, VE, "ssim" }, {
		"avg", NULL, 0, AV_OPT_TYPE_CONST, { .i64 = 1 }, INT_MIN, INT_MAX, VE,
		"ssim" }, { "frame", NULL, 0, AV_OPT_TYPE_CONST, { .i64 = 2 }, INT_MIN,
		INT_MAX, VE, "ssim" }, { "ssim_acc", "SSIM accuracy", OFFSET(ssim_acc),
		AV_OPT_TYPE_INT, { .i64 = 2 }, 0, 4, VE }, { "gmc", "use GMC", OFFSET(
		gmc), AV_OPT_TYPE_INT, { .i64 = 0 }, 0, 1, VE }, { NULL }, };

static const AVClass xvid_class = { .class_name = "libxvid", .item_name =
		av_default_item_name, .option = options, .version =
		LIBAVUTIL_VERSION_INT, };

AVCodec ff_libxvid_encoder = { .name = "libxvid", .long_name =
		NULL_IF_CONFIG_SMALL("libxvidcore MPEG-4 part 2"), .type =
		AVMEDIA_TYPE_VIDEO, .id = AV_CODEC_ID_MPEG4, .priv_data_size =
		sizeof(struct xvid_context), .init = xvid_encode_init, .encode2 =
		xvid_encode_frame, .close = xvid_encode_close, .pix_fmts =
		(const enum AVPixelFormat[] ) { AV_PIX_FMT_YUV420P,
						AV_PIX_FMT_NONE }, .priv_class = &xvid_class, };
