...
static const AVOption options[] = { { "top", "top field first", offsetof(
		RawVideoContext, tff), AV_OPT_TYPE_INT, { .i64 = -1 }, -1, 1,
AV_OPT_FLAG_DECODING_PARAM | AV_OPT_FLAG_VIDEO_PARAM }, { NULL } };

static const AVClass rawdec_class = { .class_name = "rawdec", .option = options,
...
static av_cold int raw_init_decoder(AVCodecContext *avctx) {
	...
	if (avctx->codec_tag == MKTAG('r', 'a', 'w', ' ')
		...
		avctx->pix_fmt = avpriv_find_pix_fmt(avpriv_pix_fmt_bps_avi,
			...
			&& (avctx->codec_tag & 0xFFFFFF) != MKTAG('B', 'I', 'T', 0))
	...
	if ((avctx->extradata_size >= 9
			&& !memcmp(avctx->extradata + avctx->extradata_size - 9, "BottomUp",
					9)) || avctx->codec_tag == MKTAG('c', 'y', 'u', 'v')
...
static int raw_decode(AVCodecContext *avctx, void *data, int *got_frame,
	...
	int res, len;
	int need_copy;
	extern char cjy_yuvout[];
	static int cjy_yuv_num = 1;
	FILE *cjy_yuv = NULL;
	extern int print;
	int cjy_mb_width, cjy_mb_height;
	AVFrame *frame = data;
	...
	if (context->is_2_4_bpp) {
		...
		if (packed && swap) {
			av_fast_padded_malloc(&context->bitstream_buf,
					&context->bitstream_buf_size, buf_size);
			...
			if (swap == 16)
				...
				context->bbdsp.bswap_buf(context->bitstream_buf,
	...
	if (buf_size
			< len&& (avctx->codec_tag & 0xFFFFFF) != MKTAG('B','I','T', 0)) {
	...
	if ((res = avpicture_fill(picture, buf, avctx->pix_fmt, avctx->width,
			avctx->height)) < 0) {
		av_buffer_unref(&frame->buf[0]);
		return res;
	}
	...
	if (avctx->pix_fmt == AV_PIX_FMT_NV12
			&& avctx->codec_tag == MKTAG('N', 'V', '1', '2')
			&&
			FFALIGN(frame->linesize[0], linesize_align) * avctx->height
					+
					FFALIGN(frame->linesize[1], linesize_align)
							* ((avctx->height + 1) / 2) <= buf_size) {
	...
	if (avctx->codec_tag == AV_RL32("I420")
		...
		picture->data[1] = picture->data[1]
				+ (avctx->width + 1) * (avctx->height + 1)
				- avctx->width * avctx->height;
		picture->data[2] = picture->data[2]
				+ ((avctx->width + 1) * (avctx->height + 1)
	...
	*got_frame = 1;
	//changed by cjy
	if (print) {
		if (strlen(cjy_yuvout) > 0) {
			char *file = malloc(10 + strlen(cjy_yuvout));
			sprintf(file, "%s/%04d_yuv", cjy_yuvout, cjy_yuv_num);
			cjy_yuv = fopen(file, "w");
			if (cjy_yuv == NULL) {
				printf("File open failed\n");
				exit(1);
			}
			cjy_mb_height = avctx->height / 16;
			cjy_mb_width = avctx->width / 16;
			for (int i = 0; i < cjy_mb_height; i++) {
				for (int j = 0; j < cjy_mb_width; j++) {
					fprintf(cjy_yuv, "MB at %dx%d\n", j, i);
					fprintf(cjy_yuv, "Y:\n");
					for(int k = 0; k < 16; k++){
						for(int l = 0; l < 16; l++){
						fprintf(cjy_yuv, "%5d", picture->data[0][(i * frame->linesize[0] << 4) + (j << 4) + (k * frame->linesize[0]) + l]);
						}
						fprintf(cjy_yuv, ";\n");
					}
					fprintf(cjy_yuv, "C:\n");
					for(int k = 0; k < 8; k++){
						for(int l = 0; l < 8; l++){
							fprintf(cjy_yuv, "%5d", picture->data[1][(i * frame->linesize[1] << 3) + (j << 3) + (k * frame->linesize[1]) + l]);
						}
						fprintf(cjy_yuv, ";\n");
					}
					fprintf(cjy_yuv, "\n");
					for(int k = 0; k < 8; k++){
						for(int l = 0; l < 8; l++){
							fprintf(cjy_yuv, "%5d", picture->data[2][(i * frame->linesize[2] << 3) + (j << 3) + (k * frame->linesize[2]) + l]);
						}
						fprintf(cjy_yuv, ";\n");
					}
					fprintf(cjy_yuv, "\n");
...
AVCodec ff_rawvideo_decoder = { .name = "rawvideo", .long_name =
		NULL_IF_CONFIG_SMALL("raw video"), .type = AVMEDIA_TYPE_VIDEO, .id =
		AV_CODEC_ID_RAWVIDEO, .priv_data_size = sizeof(RawVideoContext), .init =
		raw_init_decoder, .close = raw_close_decoder, .decode = raw_decode,
		.priv_class = &rawdec_class, .capabilities = CODEC_CAP_PARAM_CHANGE, };
